...
class Fraunhofer_Image:
	def __init__(self,wavelength,pixelsize,imagesize):
		...
		print('The resolution in the aperture plane is ',self.delta_x,' meters')
	...
	def make_psf(self):
		...
		self.strehl_ratio = np.amax(self.PSF)
...
class Fraunhofer_Beamfit:
	...
	def set_LMT_aperture(self,include_legs=False,plot_aperture=False,save_aperture=None):
		"""
	...
	def function2minimize(self,x,secondary_throw_array):
		...
		phase0 = self.make_phase(c=c_map0,secondary_offset=x[4]+secondary_throw_array[0],del_x=0.,del_y=0.,del_alph_x=x[2],del_alph_y=x[3],
								  f=x[1],F=525.,D=50.)
		...
		# 						  f=x[1],F=525.,D=50.)
		phase1 = self.make_phase(c=c_map1,secondary_offset=x[4]+secondary_throw_array[1],del_x=0.,del_y=0.,del_alph_x=x[2],del_alph_y=x[3],
								  f=x[1],F=525.,D=50.)
		if self.achro_beam_fit:
		...
		# 						  f=x[1],F=525.,D=50.)
		phase2 = self.make_phase(c=c_map2,secondary_offset=x[4]+secondary_throw_array[2],del_x=0.,del_y=0.,del_alph_x=x[2],del_alph_y=x[3],
								  f=x[1],F=525.,D=50.)
		if self.achro_beam_fit:
	...
	def fit_beam(self,c_guess=None,secondary_throw_array=[-1E-3,0,1E-3],boundvals = None,fit_achromatic_beam=False):
		...
		results = minimize(self.function2minimize,x0,args=secondary_throw_array,bounds=boundvals)
		...
		phase0_fit = self.make_phase(c=c_bestfit_map0,secondary_offset=results.x[4]+secondary_throw_array[0],del_x=0.,del_y=0.,del_alph_x=results.x[2],del_alph_y=results.x[3],
								  f=results.x[1],F=525.,D=50.)
		if self.achro_beam_fit:
		...
		# 						  f=results.x[1],F=525.,D=50.)
		phase1_fit = self.make_phase(c=c_bestfit_map1,secondary_offset=results.x[4]+secondary_throw_array[1],del_x=0.,del_y=0.,del_alph_x=results.x[2],del_alph_y=results.x[3],
								  f=results.x[1],F=525.,D=50.)
		if self.achro_beam_fit:
		...
		# 						  f=results.x[1],F=525.,D=50.)
		phase2_fit = self.make_phase(c=c_bestfit_map2,secondary_offset=results.x[4]+secondary_throw_array[2],del_x=0.,del_y=0.,del_alph_x=results.x[2],del_alph_y=results.x[3],
								  f=results.x[1],F=525.,D=50.)
		if self.achro_beam_fit:
	...
	def function2minimize_with_pointing_offsets(self,x,secondary_throw_array):
	...
	def fit_beam_with_pointing_offsets(self,c_guess=None,secondary_throw_array=[-1E-3,0,1E-3],boundvals = None,fit_achromatic_beam=False):
		...
		self.make_zernike_results_dict_ptingoffset()
	...
	def make_zernike_results_dict_ptingoffset(self):
		...
		zernike_values = np.array([
			...
			self.results.x[4],self.results.x[5],
			self.results.x[6],self.results.x[7],
			self.results.x[9], self.results.x[8], self.results.x[12], self.results.x[11],
			self.results.x[13], self.results.x[10], self.results.x[16],
			self.results.x[18], self.results.x[14], self.results.x[15], self.results.x[17]
		])
		results_values = np.concatenate((zernike_values,np.array([self.cost,self.gain_loss])))
		zernike_dict = {
			'labels': zernike_labels.tolist(),  # Convert NumPy array to list
			'values': results_values.tolist()   # Convert NumPy array to list
		}

		self.results_dict = zernike_dict
		print('Made Zernike dict')

	def function2minimize_focus_and_astig(self,x,secondary_throw_array):
	...
	def fit_beam_focus_and_astig(self,c_guess=None,secondary_throw_array=[-1E-3,0,1E-3],boundvals = None,fit_achromatic_beam=False):
		...
		x0 = np.zeros(10) # need to make an array that has the zernike coeffs plus 1 for theta0
		x0[0] = np.amax(self.trunc_maps['map1'])
		...
		self.fit_step_counter = 0
		results = minimize(self.function2minimize_focus_and_astig,x0,args=secondary_throw_array,bounds=boundvals)
		self.results = results
		if results.success:
			print('The fit was successful. Cost =', self.cost)
		else:
	...
	def function2minimize_focus_and_astig_M2included(self,x,secondary_throw_array):
		...
		phase0 = self.make_phase(c=c_map0,secondary_offset=x[1]+secondary_throw_array[0],del_x=x[2],del_y=x[3],del_alph_x=0.,del_alph_y=0.,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			model_map0 = self.make_psf(phase=phase0,return_psf=True)*x[0]
		else:
		...
		# phase1 = self.make_phase(c=c_map1,secondary_offset=x[4],del_x=x[3],del_y=x[4],del_alph_x=0.,del_alph_y=0.,
		# 						  f=x[1],F=525.,D=50.)
		phase1 = self.make_phase(c=c_map1,secondary_offset=x[1]+secondary_throw_array[1],del_x=x[2],del_y=x[3],del_alph_x=0.,del_alph_y=0.,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			model_map1 = self.make_psf(phase=phase1,return_psf=True)*x[0]
		else:
		...
		# phase2 = self.make_phase(c=c_map2,secondary_offset=x[4]+1E-3,del_x=x[3],del_y=x[4],del_alph_x=0.,del_alph_y=0.,
		# 						  f=x[1],F=525.,D=50.)
		phase2 = self.make_phase(c=c_map2,secondary_offset=x[1]+secondary_throw_array[2],del_x=x[2],del_y=x[3],del_alph_x=0.,del_alph_y=0.,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			model_map2 = self.make_psf(phase=phase2,return_psf=True)*x[0]
		else:
	...
	def fit_beam_focus_and_astig_M2included(self,c_guess=None,secondary_throw_array=[-1E-3,0,1E-3],boundvals = None,fit_achromatic_beam=False):
		...
		# phase0_fit = self.make_phase(c=c_bestfit_map0,secondary_offset=results.x[4]-1.E-3,del_x=results.x[3],del_y=results.x[4],del_alph_x=0.,del_alph_y=0.,
		# 						  f=results.x[1],F=525.,D=50.)
		phase0_fit = self.make_phase(c=c_bestfit_map0,secondary_offset=results.x[1]+secondary_throw_array[0],del_x=results.x[2],del_y=results.x[3],del_alph_x=0,del_alph_y=0,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			tmppsf_raw = self.make_psf(phase=phase0_fit,return_psf=True)*results.x[0]
		...
		# phase1_fit = self.make_phase(c=c_bestfit_map1,secondary_offset=results.x[4],del_x=results.x[3],del_y=results.x[4],del_alph_x=0.,del_alph_y=0.,
		# 						  f=results.x[1],F=525.,D=50.)
		phase1_fit = self.make_phase(c=c_bestfit_map1,secondary_offset=results.x[1]+secondary_throw_array[1],del_x=results.x[2],del_y=results.x[3],del_alph_x=0,del_alph_y=0,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			tmppsf_raw = self.make_psf(phase=phase1_fit,return_psf=True)*results.x[0]
		else:
		...
		# phase2_fit = self.make_phase(c=c_bestfit_map2,secondary_offset=results.x[4]+1.E-3,del_x=results.x[3],del_y=results.x[4],del_alph_x=0.,del_alph_y=0.,
		# 						  f=results.x[1],F=525.,D=50.)
		phase2_fit = self.make_phase(c=c_bestfit_map2,secondary_offset=results.x[1]+secondary_throw_array[2],del_x=results.x[2],del_y=results.x[3],del_alph_x=0,del_alph_y=0,
								  f=17.5,F=525.,D=50.)
		if self.achro_beam_fit:
			tmppsf_raw = self.make_psf(phase=phase2_fit,return_psf=True)*results.x[0]
		else:
		...
		phase_ideal_fit = self.make_phase(c=c_bestfit_ideal,secondary_offset=0.,del_x=results.x[2],del_y=results.x[3],del_alph_x=0,del_alph_y=0,
								  f=17.5,F=525.,D=50.)
		self.phase = phase_ideal_fit
